...
**错误信息**:
```
Error: Cannot find module '@because/api/dist/index.js'
```
...
**说明**: 
- `agents-because` 的类型声明文件（`.d.ts`）是可选的
...
**解决方案**:
1. 访问 https://because.ai/toolkit/creds_generator 生成密钥
2. 在 `.env` 文件中设置：
   ```
...
**解决方案**:
- 查看更新日志: https://because.ai/changelog
- 根据需要更新 `Because.yaml` 配置
