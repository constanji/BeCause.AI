...
├── package.json
├── src/
└── agents-because/    # 把 agents-because 放在这里
    ├── package.json
    └── ...
```
...
```bash
# 在你的项目根目录中
npm install ./agents-because

# 或者使用绝对路径
npm install /absolute/path/to/agents-because
```

#### 方案 C：使用 npm link（跨目录）

```bash
# 1. 在 agents-because 目录中
cd /path/to/agents-because
npm install
npm run build
npm link
...
**注意**：当前包名 `@because/agents` 包含大写字母，npm 不再允许。如果要发布，需要修改包名：

1. 修改 `package.json` 中的 `name` 为小写，例如：`@because/agents`
2. 然后发布：`npm publish`
...
```typescript
import { Run } from '@because/agents';
import { HumanMessage } from '@langchain/core/messages';
import { getLLMConfig } from '@because/agents/utils/llmConfig';
...
```typescript
import { Run, Calculator } from '@because/agents';
import { HumanMessage } from '@langchain/core/messages';
import { getLLMConfig } from '@because/agents/utils/llmConfig';
import { Providers } from '@because/agents';
...
```typescript
import { Run, GraphEvents, ChatModelStreamHandler } from '@because/agents';
import { HumanMessage } from '@langchain/core/messages';
import { getLLMConfig } from '@because/agents/utils/llmConfig';
import { Providers } from '@because/agents';
