...
3. **可定制**：通过变量替换机制实现个性化
4. **易用性**：模板简单易懂，易于使用
5. **RAG集成**：深度集成RAG知识检索服务，自动检索语义模型、QA对、同义词、业务知识等

## RAG 服务集成

本系统已深度集成 RAG（检索增强生成）服务，在执行命令前会自动检索相关知识：
...
1. **向量化**：将用户查询转换为向量嵌入
2. **检索**：从知识库中检索相关语义模型、QA对、同义词、业务知识
3. **重排**：使用重排模型优化检索结果
4. **整合**：将检索到的知识整合到提示词中
...
在执行任何命令前，系统会自动：
1. 调用 RAG 服务检索相关知识（`POST /api/rag/query`）
2. 根据检索结果自动填充变量：
   - `rag_semantic_models`：检索到的语义模型
   - `rag_qa_pairs`：相关的QA对
   - `rag_synonyms`：相关的同义词
...
- **[执行流程文档](./WORKFLOW.md)**：强制执行流程说明，每轮对话必须遵守
- [工具使用说明](./TOOL_USAGE.md)：工具的使用方法和参数说明
- [使用指南](./USAGE_GUIDE.md)：详细的使用指南和示例
...
- `{{ query }}`：用户查询
- `{{ query_time }}`：当前时间
- `{{ semantic_models }}`：语义模型列表（传统方式，已废弃，优先使用RAG检索）
- `{{ language }}`：输出语言

### RAG 检索占位符（自动填充）
- `{{ rag_semantic_models }}`：RAG检索到的语义模型（推荐使用）
- `{{ rag_qa_pairs }}`：RAG检索到的相关QA对
- `{{ rag_synonyms }}`：RAG检索到的同义词
- `{{ rag_business_knowledge }}`：RAG检索到的业务知识
- `{{ rag_results }}`：完整的RAG检索结果（包含所有类型）
...
- `{{ data_samples }}`：数据样本
- `{{ sql_samples }}`：SQL 示例
- `{{ synonyms }}`：同义词列表（传统方式，已废弃，优先使用RAG检索）
- `{{ docs }}`：业务知识文档（传统方式，已废弃，优先使用RAG检索）
- `{{ histories }}`：查询历史
