 ...
 * Priority:
 * 1. BECAUSE_LOG_DIR environment variable
 * 2. If running within Because monorepo (when cwd ends with /api), use api/logs
...
export const getLogDirectory = (): string => {
  if (process.env.BECAUSE_LOG_DIR) {
    return process.env.BECAUSE_LOG_DIR;
  }
  ...
  // For Because project structure, use api/logs
  // For external consumers, they should set BECAUSE_LOG_DIR
  if (cwd.includes('Because')) {
    return apiLogsPath;
  }
